  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials): Promise<ExtendedUser | null> {
        ...
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://cto-backend-production-28e3.up.railway.app'}/api/auth/login`, {
            method: 'POST',
            headers: {
...
async function refreshAccessToken(token: ExtendedToken): Promise<ExtendedToken> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://cto-backend-production-28e3.up.railway.app'}/api/auth/refresh`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
